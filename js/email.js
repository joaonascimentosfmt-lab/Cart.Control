...
function isAlert(e) {
  ...
  return (Date.now() - sent.getTime()) > ALERT_HOURS * 3600 * 1000;
}
...
export function renderEmail(root) {
  ...
  root.innerHTML = `
    ...
    ${alerts.length > 0
      ? `<div class="alert-banner">&#9888; ${alerts.length} e-mail(s) sem abertura há mais de ${ALERT_HOURS} horas.<span>Pode ter passado despercebido — verifique abaixo e considere reenviar ou contato por telefone.</span></div>`
      : ""}

    <div class="card">
      <div class="toolbar">
        <button class="btn primary" id="email-add">+ Registrar e-mail enviado</button>
      </div>
  ...
  function draw() {
    ...
    if (sorted.length === 0) {
      list.innerHTML = `<div class="empty"><span class="big">&#9993;</span>Nenhum e-mail registrado.</div>`;
      return;
    }
        ...
        <div class="email-item ${alert ? "alert" : ""}" data-id="${e.id}">
          ...
          <div class="foot">
            <span class="status-dot ${cls}">
              ${alert ? `&#9888; Não aberto há ${timeAgo}` : STATUS_LABEL[e.status]}
            </span>
    ...
    <div class="modal" role="dialog" aria-modal="true">
      ...
      <form class="form-grid" id="email-form">
        ...
        <div class="form-field">
          ...
          <input type="datetime-local" name="sentAt" value="${e.sentAt.slice(0, 16)}" required>
        </div>
        <div class="form-field">
          <label>Status</label>
          <select name="status">

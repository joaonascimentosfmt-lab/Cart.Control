  ...
  function renderPlanilha(root) {
    ...
    <div class="card">
      <div class="toolbar">
        ...
        <button class="btn primary" id="planilha-add">${icon("plus")} Novo protocolo</button>
      </div>
    ...
    function draw() {
      ...
      const filtered = rows.filter((r) => {
        ...
        const matchF = !f || calcStatus(r) === f;
        return matchQ && matchF;
      }).sort((a, b) => b.protocolo.localeCompare(a.protocolo));

      if (filtered.length === 0) {
        tbody.innerHTML = `<tr><td colspan="8"><div class="empty"><span class="big">${icon("clipboard")}</span>Nenhum protocolo encontrado.</div></td></tr>`;
        return;
      }

      tbody.innerHTML = filtered.map((r) => {
        const cs = calcStatus(r);
        const diff = (new Date(r.prazo + "T00:00:00") - today) / 86400000;
        const cls = cs;
        const pillLabel = cs === "finalizado" ? "Finalizado" : cs === "vencido" ? `Vencido há ${Math.abs(diff)}d` : cs === "risco" ? `Em risco (${Math.abs(diff)}d)` : "No prazo";
        return `
        <tr data-id="${r.id}">
          ...
          <td><div class="row-actions">
            <button class="icon-btn" data-edit="${r.id}" title="Editar">${icon("edit")}</button>
            <button class="icon-btn" data-del="${r.id}" title="Excluir">${icon("trash")}</button>
          </div></td>
        </tr>`;
      }).join("");
    }

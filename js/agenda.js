(function () {
  ...
  function renderAgenda(root, dateISO) {
    ...
    <div class="card">
      <div class="date-nav">
        <button data-nav="-1" aria-label="Dia anterior">${icon("chevron-left")}</button>
        <div class="date-label">${esc(weekdayLabel(dateISO))}</div>
        <button data-nav="1" aria-label="Próximo dia">${icon("chevron-right")}</button>
      </div>
      <div class="slots">
        ${SLOTS.map((time) => {
          const a = map[time];
    ...
    <div class="card">
      ...
      ${list.length === 0
        ? `<div class="empty"><span class="big">${icon("calendar")}</span>Nenhum agendamento para esta data.<br>Toque em um horário livre para agendar.</div>`
              ...
              <div class="appointment ${a.status}">
                ...
                <div style="display:flex;flex-direction:column;align-items:flex-end;gap:6px;">
                  ...
                  <div class="row-actions">
                    ...
                    <button class="icon-btn" data-confirm="${a.id}" title="Confirmar">${icon("check")}</button>
                    <button class="icon-btn" data-done="${a.id}" title="Concluir">${icon("check-circle")}</button>
                    <button class="icon-btn" data-cancel="${a.id}" title="Cancelar">${icon("x")}</button>
                  </div>
                </div>
              </div>`).join("")}
          </div>`}
